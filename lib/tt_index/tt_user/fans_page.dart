...
import '../../../bloc.dart';
import '../../../base.dart';
class UserFansPage extends StatefulWidget with TTBase {
  final arguments;
  UserFansPage({Key key, this.arguments}) : super(key: key);
  @override
  _UserFansPageState createState() => _UserFansPageState();
}

class _UserFansPageState extends State<UserFansPage> with SingleTickerProviderStateMixin,TTBase {
  ...
  void loadFansList(bool clearData) async {
    if (loading) {
      return;
    }
    loading = true;
    var res = await TTService.userFans(offset, user == null ? '' : user.id);
    if (userList == null) {
      userList = new List();
    }
    if (clearData) {
    ...
    if (res['code'] == 1) {
      if((res['data']['fans'] as List).length==0){
        //没有更多了
        _refreshController.loadNoData();
      }
      (res['data']['fans'] as List).forEach((element) {
        ...
        builder: (context, indexState) {
          return Scaffold(
              ...
              appBar: new AppBar(
                ...
                title: Text(
                  user == null ? '我的粉丝' : 'TA的粉丝', style: TextStyle(color: Theme
                    .of(context)
                    .textTheme
                    .bodyText1
                    .color),),
  ...
  _list() {
    ...
    return
      ...
      Column(crossAxisAlignment: CrossAxisAlignment.center,
          ...
          ]) : (userList.length == 0 ? Center(child: Text('还没有任何人关注')) :
      Container(
