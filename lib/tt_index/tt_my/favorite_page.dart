...
import '../../../bloc.dart';
import '../../../base.dart';
class FavoritePage extends StatefulWidget with TTBase {
  @override
  _FavoritePageState createState() => _FavoritePageState();
}

class _FavoritePageState extends State<FavoritePage> with SingleTickerProviderStateMixin,TTBase {
  ...
  void loadVideoList(bool clearData) async {
    ...
    loading = true;
    var res = await TTService.favoriteList(
    ...
    if (res['code'] == 1) {
      (res['data']['favorites'] as List).forEach((element) {
  ...
  void deleteAllHistory() {
    _loadingCallBack(Function func) async {
      var res = await TTService.removeFavorite('', true, []);
      if (res['code'] != 1) {
        func();
    ...
    TTDialog.showCustomDialog(context, text: '确定清空所有收藏吗？清空后将永久无法找回，请谨慎操作。',
        yes: '清空',
  ...
  void deleteSelectedHistory() async {
    _loadingCallBack(Function func) async {
      var res = await TTService.removeFavorite('', false, deleteHashs);
      if (res['code'] != 1) {
        func();
    ...
    TTDialog.showCustomDialog(
        context, text: '确定删除' + deleteHashs.length.toString() + '条收藏吗？',
        yes: '删除',
        okCallBack: _okCallBack);
  }
        ...
        builder: (context, indexState) {
          return Scaffold(
              ...
              appBar: new AppBar(
                ...
                title: Text('我的收藏', style: TextStyle(color: Theme
                    .of(context)
                    .textTheme
                    .bodyText1
                    .color),),
  ...
  _list() {
    ...
    return
      ...
      Column(crossAxisAlignment: CrossAxisAlignment.center,
          ...
          ]) : (videoList.length == 0 ? Center(child: Text('尚未收藏任何内容')) :
      Container(
